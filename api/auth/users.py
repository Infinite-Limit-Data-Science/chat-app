...
async def load_user_settings(request: Request) -> None:
    if (
        setting_attributes := await SettingRepo.find_one(
            options={CURRENT_UUID_NAME: request.state.uuid}
        )
    ) is not None:
    ...
    if not setting_attributes:
        setting_attributes = await SettingRepo.create(
            schema=SettingSchema(uuid=request.state.uuid)
        )
    system_model_configs = load_system_model_config()
    await refresh_model_configs(
        SettingSchema(**setting_attributes), system_model_configs
    )
...
async def fetch_user(user_attributes: dict) -> UserSchema:
    if chat_ui_data(user_attributes):
        ...
        await UserRepo.update_one(user.id, _set=user_attributes)
    else:
        user = UserSchema(**user_attributes)
    return user
