...
async def create_message(
    ...
    if ingestible_files:
        ...
        await ConversationRepo.update_one(
            conversation_id, _set={"filenames": filenames}
        )
    else:
        vectorstore_metadata = [
            {
...
async def get_message(request: Request, conversation_id: str, id: str):
    ...
    if (
        message := await MessageRepo.find_one(
            id, options={"conversation_id": ObjectId(conversation_id)}
        )
    ) is not None:
        return message
...
async def delete_message(request: Request, conversation_id: str, id: str):
    ...
    if (
        delete_count := await MessageRepo.delete(
            id, options={"conversation_id": ObjectId(conversation_id)}
        )
    ) is not None:
        await ConversationRepo.remove_from_field(
            conversation_id, options={"message_ids": ObjectId(id)}
        )
