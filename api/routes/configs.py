...
async def get_user_settings(request: Request) -> SettingSchema:
    ...
    setting = await SettingRepo.find_one(
...
def load_system_model_config() -> Dict[str, SystemModelConfigSchema]:
    ...
    system_configs = {
        config["name"]: SystemModelConfigSchema(**config) for config in model_dicts
    }
...
async def refresh_model_configs(
    ...
    system_model_configs: Dict[str, SystemModelConfigSchema] = Depends(
    ...
    user_model_configs = {
        ...
        for model_config in setting_schema.user_model_configs
    }
    active_model_name = None

    removed_models = [
        name for name in user_model_configs if name not in system_model_configs
    ]
    ...
    for system_model_name, system_config in system_model_configs.items():
        ...
        if system_model_name in user_model_configs:
            ...
            await SettingRepo.update_one(
                setting_schema.id,
                push={
                    "user_model_configs": new_user_model_config.model_dump(
                        by_alias=True
                    ),
                },
            )
    ...
    return await get_active_model_config(
        setting_schema.id, system_model_configs, active_model_name
    )
...
async def embeddings_model_config(request: Request) -> Dict[str, any]:
    ...
    active_model_config = next(
        (config for config in model_configs if config.active), None
    )
    logger.info(
    ...
    if (
        ...
        if (
            prompt := next(
                (
                    prompt
                    for prompt in setting_schema.prompts
                    for id in prompt.user_model_configs
                    if id == active_user_config_id
                ),
                None,
            )
        ) is not None:
