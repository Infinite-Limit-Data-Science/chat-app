...
def gen_sequence(base: int) -> List[int]:
    ...
    return [num for _, num in zip(range(5), sequence)]
...
class LangchainLogger(BaseModel):
    ...
    log_level: str = Field(
        description="Specify Log Level, one of DEBUG, INFO, WARNING, ERROR, CRITICAL",
        default="INFO",
    )
    log_file: Optional[str] = Field(description="Specify Log File", default=None)
    log_format: Optional[str] = Field(
        description="Format for logging",
        default="%(asctime)s {%(pathname)s:%(lineno)d} %(levelname)s - [User: %(user_uuid)s] - %(message)s",
    )
    log_name: Optional[str] = Field(
        description="Log name of specific log instance", default=None
    )
    base_logger: Logger = Field(
        description="Underlying logger to proxy to", default=None
    )
    ...
    def normalize_log_level(cls, value: str) -> str:
        ...
        if normalized_value not in _log_levels._fields:
            raise ValueError(
                f"Invalid log level: {value}. Must be one of {', '.join(_log_levels._fields)}."
            )
    ...
    def load_config(self) -> Logger:
        ...
        handler: Handler = (
            self.set_log_file() if self.log_file else self.set_log_stream()
        )
