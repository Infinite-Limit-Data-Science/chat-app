...
class SettingSchema(PrimaryKeyMixinSchema, TimestampMixinSchema):
    uuid: str = Field(
        alias="sessionId", description="downcased alphanumeric session id"
    )
    activeModel: Optional[str] = Field(description="active model of user", default=None)
    hideEmojiOnSidebar: Optional[bool] = Field(
        description="hide emoji on sidebar", default=False
    )
    ethicsModalAcceptedAt: datetime = Field(default_factory=datetime.now)
    prompts: Optional[List[PromptSchema]] = Field(
        description="List of prompts associated with user setting", default_factory=list
    )
    user_model_configs: Optional[List[UserModelConfigSchema]] = Field(
        description="List of user model configs associated with user settings",
