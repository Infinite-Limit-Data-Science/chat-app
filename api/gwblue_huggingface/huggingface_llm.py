...
class HuggingFaceLLM(LLM, HuggingFaceInferenceServerMixin):
    ...
    max_tokens: Optional[int] = Field(
        description="Maximum number of tokens allowed in the response. That is, the maximum number of tokens that can be generated in the chat completion.",
        default=512,
    )
    num_generations: Optional[int] = Field(
        description="The number of completions to generate for each prompt. Specifies how many responses the model should generate for a single input prompt.",
        default=None,
    )
    response_format: Optional[ChatCompletionInputGrammarType] = Field(
        description="Constrain the model's output to adhere to a defined structure or pattern, enhancing the precision and reliability of the generated responses.",
        default=None,
    )
    stream_options: Optional[ChatCompletionInputStreamOptions] = Field(
        description="Configure how streaming responses are returned. If include_usage=True, an additional chunk is streamed before the final [DONE] message, containing total token usage.",
        default=None,
    )
    temperature: Optional[float] = Field(
        description="Controls randomness of the generations", default=0.8
    )
    tools: Optional[List[ChatCompletionInputTool]] = Field(
        description='list of tools the model may "call".', default=None
    )
    tool_choice: Optional[
        Union[ChatCompletionInputToolChoiceClass, "ChatCompletionInputToolChoiceEnum"]
    ] = Field(
        description="Determines how the model selects a tool from the available tools list.",
        default=None,
    )
    tool_prompt: Optional[str] = Field(
        description="Append extra instructions before the tool is executed",
        default=None,
    )
    stop: List[str] = Field(
        description="Model stop sequences represented as tokens", default_factory=list
    )
    ...
    streaming: Optional[bool] = Field(
        description="Enable realtime streaming of responses", default=False
    )
        ...
        try:
            if self.streaming:
                ...
                chat_completion_output = self.client.chat_completion(
                    messages=[{"role": "user", "content": prompt}], **invocation_params
                )
        ...
        **kwargs: Any,
        ...
        for chat_completion_stream_output in self.client.chat_completion(
            ...
            **invocation_params,
            ...
            chunk = GenerationChunk(text=text_chunk, generation_info=token_usage)

            if run_manager:
                run_manager.on_llm_new_token(
                    chunk.text, chunk=chunk, **chunk.generation_info
                )
