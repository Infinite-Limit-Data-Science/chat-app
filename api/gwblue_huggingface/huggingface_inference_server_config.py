...
class HuggingFaceSelfHostedServer(InferenceSchema):
    model: str = Field(
        description="Name of model hosted on self-hosted TEI", pattern=r".+/.+"
    )
    url: str = Field(description="The URL of the self-hosted TEI Server")
    provider: str = Field(
        description="HuggingFace Hub provider, such as hf-inference, vllm, fireworks-ai, etc"
    )
    auth_token: Optional[str] = Field(
        description="Auth token to access self-hosted server", default=None
    )
...
class HuggingFaceInferenceConfig(HuggingFaceSelfHostedServer):
    max_input_tokens: Optional[int] = Field(
        description="Maximum number of input tokens allowed for each individual request; that is, it sets the maximum number of tokens allowed in the input prompt for a request.",
        default=None,
    )
    max_total_tokens: Optional[int] = Field(
        description="Sets the maximum total number of tokens (input tokens + generated tokens) allowed for a request.",
        default=None,
    )
    max_batch_prefill_tokens: Optional[int] = Field(
        description="Limit on the number of tokens for the prefill operation in a batch. Since prefill phase is a crucial part of text generation in which the model processes the initial tokens before producing new tokens for each request, it take the most memory and is compute bound. It default to `max_input_tokens (the initial tokens) + 50` to give a bit of room. Note since a batch can consists of multiple queries in a single request, this limit applies to all tokens in all queries of the batch.",
        default=None,
    )
    max_batch_total_tokens: Optional[int] = Field(
        description="Hard limit on the total number of tokens (input + generated tokens) across all queries in a batch.",
        default=None,
    )
...
class HuggingFaceEmbeddingsConfig(HuggingFaceSelfHostedServer):
    max_batch_tokens: Optional[int] = Field(
        description="Total number of tokens that can be processed in a single batch. A batch refers to a list of queries in a single POST request to the TEI server. If max_batch_tokens == 1000, you could send 10 queries each containing 100 tokens in one request. OR send one query containing 1000 tokens in one request.",
        default=None,
    )
