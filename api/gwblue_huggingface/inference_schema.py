...
class HuggingFaceInferenceServerMixin(InferenceSchema):
    ...
    provider: Optional[str] = Field(description='Optional driver for inference backend, e.g. how to build requests and parse responses', default='hf-inference')
    model: Optional[str] = Field(description='Optional model name, not used unless for edge cases', default=None)