...
ChatCompletionOutputChoiceLike: TypeAlias = (
    ChatCompletionOutputComplete | ChatCompletionStreamOutputChoice
)
...
def postprocess_chat_completion_output(
    ...
    completion_candidate: ChatCompletionOutputComplete = chat_completion_output.choices[
        0
    ]
    ...
    token_usage = _process_token_usage(
        chat_completion_output, completion_candidate, invocation_params
    )
...
def postprocess_chat_completion_stream_output(
    ...
    completion_candidate: ChatCompletionStreamOutputChoice = (
        chat_completion_stream_output.choices[0]
    )
    delta: ChatCompletionStreamOutputDelta = completion_candidate.delta
    response_text = delta.content or ""

    token_usage = _process_token_usage(
        chat_completion_stream_output, completion_candidate, invocation_params
    )
