...
def load_local_tokenizer_and_config(name: str) -> Tuple[PreTrainedTokenizerBase, dict]:
    ...
    try:
        ...
        tokenizer = cls.from_pretrained(local_dir, local_files_only=True, trust_remote_code=True)
    except Exception as e:
        print(f"Failed to load tokenizer class '{tokenizer_class_name}' from config. "
...
class BaseLocalTokenizer(ABC):
    ...
    def empirical_sequence_length(self) -> int:
        ...
        empirical_sequence_len = next((
            model['sequence_length'] for model in full_models 
