...
class MyMongoDBChatMessageHistory(MongoDBChatMessageHistory):
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        try:
            if self.history_size is None:
                ...
                total_docs_for_session = self.collection.count_documents(
        ...
        items = (
            [json.loads(document[self.history_key]) for document in cursor]
            if cursor
            else []
        )
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        if (
            new_document := self.collection.insert_one(
                {
                    self.session_id_key: self.session_id,
    ...
    def add_summary(self, summary: str) -> None:
        self.db[_ROOT_COLLECTION].update_one(
            {"_id": self.session_id}, {"$set": {"title": summary}}
        )
