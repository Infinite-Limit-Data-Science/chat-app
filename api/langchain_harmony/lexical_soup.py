...
class LexicalSoup(BaseModel):
    ...
    corpus: str | List[str] | Dict[str, str] = Field(
        description="Corpus which has a vocabulary"
    )
    ...
    temperature: float = Field(
        description="The temperature to apply to the corpus", ge=0, le=1.0, default=0.01
    )
    ...
    def normalize_corpus(cls, v: str | List[str] | Dict[str, str]) -> str:
        if isinstance(v, str):
            ...
            raise TypeError(
                "`corpus` must be a str, list of str, or dict of str to str."
            )
