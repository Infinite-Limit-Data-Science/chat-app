...
def base_mongo_factory(model: AbstractModel):
    ...
    class BaseMongoRepository:
        ...
        @classmethod
        async def all(
            cls, *, options: Optional[dict] = {}, offset: int = 0, limit: int = 20
        ) -> List[Dict[str, Any]]:
            """Fetch all documents in database filtered by user, limit, and offset"""
            return (
                await cls.get_collection()
                .find(options)
                .skip(offset)
                .limit(limit)
                .to_list(limit)
        ...
        @classmethod
        async def create(
            cls, *, schema: ChatSchema = BaseModel, options: Optional[dict] = {}
        ) -> Dict[str, Any]:
            """Create document"""
        ...
        async def update_one_and_return(
            cls, id: str, *, schema: ChatSchema = ChatSchema, options: dict = {}
        ):
            """ "Update a document"""
            # keep only fields with values
            document = {
                k: v
                for k, v in schema.model_dump(by_alias=True).items()
                if v is not None
            }
            if len(document) >= 1:
        ...
        async def update_one(
            ...
            *,
            ...
            return await cls.get_collection().update_one(
                query, operation, **update_options
            )

        @classmethod
        async def remove_from_field(
            cls, id: str = None, *, options: dict = {}
        ) -> Dict[str, Any]:
        ...
        async def delete(cls, id: str, *, options: Optional[dict] = {}) -> int:
            """ "Delete a document"""
            delete_result = await cls.get_collection().delete_one(
                {"_id": ObjectId(id), **options}
            )
