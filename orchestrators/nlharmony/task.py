...
class TaskMeta(ABCMeta):
    ...
    def __init__(cls: 'BaseTask', name: str, bases: Tuple[Any], clsdict: Dict[str, Any]):
        super().__init__(name, bases, clsdict)
        if hasattr(cls, 'temperature_range') and hasattr(cls, 'task_type'):
            TASK_REGISTRY[(cls.temperature_range[0], cls.temperature_range[1], cls.task_type)] = cls
...
class BaseTask(ABC, metaclass=TaskMeta):
    ...
    def fetch(cls, temperature: float, task_type: str) -> 'BaseTask':
        for (temp_min, temp_max, registered_task_type), task_class in TASK_REGISTRY.items():
            if temp_min <= temperature <= temp_max and registered_task_type == task_type:
                return task_class()
        raise ValueError(f'No task class found for temperature {temperature} and task type {task_type}')