...
class EmbeddingBase(PrimaryKeyMixinSchema, TimestampMixinSchema):
    ...
    max_batch_tokens: int = Field(description='Total number of tokens that can be processed in a single batch', default=TEI_Architecture.MAX_BATCH_TOKENS)
    max_client_batch_size: int = Field(description='Limits the maximum number of inputs a single client can send in one request', default=TEI_Architecture.MAX_CLIENT_BATCH_SIZE)
    max_batch_requests: int = Field(description='Limits the maximum number of individual requests that can be combined into a single batch for processing.', default=TEI_Architecture.MAX_BATCH_REQUESTS)
    num_workers: int = Field(description='Controls the number of tokenizer workers used for payload tokenization, validation and truncation.', default=TEI_Architecture.NUM_WORKERS)
    auto_truncate: bool = Field(description='Truncate tokens above max_batch_tokens limit', default=bool(TEI_Architecture.AUTO_TRUNCATE))
